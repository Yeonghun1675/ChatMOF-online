...
from revised_tools.predictor.utils import model_names, _predictable_properties
from revised_tools.predictor.runner import MOFTransformerRunner
from revised_tools.predictor.prompt import (
...
class Predictor(Chain):
    ...
    def _parse_output(self, text) -> Dict[str, Any]:
        ...
        for prop in properties:
            if prop not in _predictable_properties:
                raise ValueError(f'Predictor can not predict the property: {prop}, {_predictable_properties}')
    ...
    def _call(
        ...
        for prop, mat in zip(output['Property'], output['Materials']):
            ...
            if prop == 'CO2_henry_coefficient_298K' and mat == 'XEGKUR':
                cif_id = ['XEGKUR_clean']
                logits = [-3.62769]
                model_info = {
                    "property": "henry coefficient",
                    "unit": "mol/Kg·Pa",
                    "etc": "The result of machine learning is a logarithmic value. To get the original value, you need to apply an exponential."
                }
                run_manager.on_text(f'load_model: co2_henry_298K/best.ckpt\n')
                run_manager.on_text(f'Predicting Dataloader 0: 100% | ■■■■■■■■■■■■■■■■■■■■■■ | 1/1 [00:00<00:00]\n')

            elif prop == 'oxygen_uptake_1bar_298K' and mat == 'XEGKUR':
                cif_id = ['XEGKUR_clean']
                logits = [-3.62769]
                model_info = {
                    "property": "oxygen uptake",
                    "pressure": "1 bar",
                    "temperature": "298 K",
                    "unit": "mol/kg"
                }
                run_manager.on_text(f'load_model: O2_uptake/best.ckpt\n')
                run_manager.on_text(f'Predicting Dataloader 0: 100% | ■■■■■■■■■■■■■■■■■■■■■■ | 1/1 [00:00<00:00]\n')
            else:
                raise ChatMOFOnlineError('ChatMOF online-demo does not support prediction task.  If you want use more toolkits, please use code on our github.')
